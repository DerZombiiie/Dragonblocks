...
var agag=(gravity-map[vals[4]][vals[5]].up)*vals[0];
var agbg=(gravity-map[vals[4]][vals[7]].up)*vals[1];
var bgag=(gravity-map[vals[6]][vals[5]].up)*vals[2];
var bgbg=(gravity-map[vals[6]][vals[7]].up)*vals[3];	
...
var brakeFall=akak+bkak+akbk+bkbk;
		...
		yvelocity=(Math.pow(time,2)*(usegravity/frequency)/2)*lame + startyvel * time;
		yvelocity=yvelocity*brakeFall;
		
		if((used<0&&usegravity>0)||(used>0&&usegravity<0)){
			time = 1/frequency;
			}
		if(usegravity === 0) {
			time = 1/frequency;
	...
	if((ypos+yvelocity)>high){	
		...
		fto=setTimeout(fall, 1000/frequency,usegravity);
	...
	function acc(evt){
		...
		if(evt.key==="ArrowLeft"&&a&&LeftArrowEvent){
			...
			var brakeMov=akak+bkak+akbk+bkbk;
					xvelocity = -velocity*brakeMov/frequency;	
...
to=setTimeout(acc, 1000/frequency,evt);
	...
	if(evt.key==="ArrowRight"&&a&&RightArrowEvent){
	...
	if(evt.keyCode==32&&SpaceEvent){
		
	evt.preventDefault();
	if(map[Math.floor(xpos)][Math.floor(ypos)+1].mobStability||map[Math.ceil(xpos)][Math.floor(ypos)+1].mobStability){
	yvelocity=-jumpspeed/frequency;
	
		
		}}
	if(evt.key==="Shift"&&ShiftEvent){	
	for(i=0;i<shiftPhysicsBlocks.length;i++){
			shiftPhysicsBlocks[i].orginup = shiftPhysicsBlocks[i].up;
			shiftPhysicsBlocks[i].up = shiftPhysicsBlocks[i].shiftup;
			}	
		}			
	}
		...
		if(evt.key==="Shift"){
		for(i=0;i<shiftPhysicsBlocks.length;i++){
			shiftPhysicsBlocks[i].up = shiftPhysicsBlocks[i].orginup;
			}
		}		
			}
